...
def create_category(category_data: ProductCategoryCreate, db: Session = Depends(get_db)):
...
def update_category(
    ...
    db_category = crud.update_category(db=db, category_id=str(category_id), category_data=category_data)
    if db_category is None:
        raise HTTPException(status_code=404, detail="Category not found")
    return db_category
