...
from app.crud.order import create_order, update_order_status, delete_order, list_orders, get_order
...
def update_order_status_endpoint(order_id: str, status: OrderStatusUpdate, db: Session = Depends(get_db)):
    updated_order = update_order_status(db=db, order_id=order_id, status=status)
